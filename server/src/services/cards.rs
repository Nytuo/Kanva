...
pub async fn delete_card(state: &AppState, _user_id: Uuid, card_id: Uuid) -> Result<(), AppError> {
...
pub async fn assign_card(state: &AppState, _user_id: Uuid, card_id: Uuid, assignee_id: Uuid) -> Result<(), AppError> {
    sqlx::query(&state.q("INSERT INTO card_assignees (card_id, user_id) VALUES (?, ?) ON CONFLICT DO NOTHING"))
        .bind(card_id.to_string()).bind(assignee_id.to_string()).execute(&state.db).await?;
    Ok(())
}

pub async fn unassign_card(state: &AppState, _user_id: Uuid, card_id: Uuid, assignee_id: Uuid) -> Result<(), AppError> {
    sqlx::query(&state.q("DELETE FROM card_assignees WHERE card_id = ? AND user_id = ?"))
        .bind(card_id.to_string()).bind(assignee_id.to_string()).execute(&state.db).await?;
    Ok(())
}

pub async fn add_label(state: &AppState, _user_id: Uuid, card_id: Uuid, label_id: Uuid) -> Result<(), AppError> {
    sqlx::query(&state.q("INSERT INTO card_labels (card_id, label_id) VALUES (?, ?) ON CONFLICT DO NOTHING"))
        .bind(card_id.to_string()).bind(label_id.to_string()).execute(&state.db).await?;
    Ok(())
}

pub async fn remove_label(state: &AppState, _user_id: Uuid, card_id: Uuid, label_id: Uuid) -> Result<(), AppError> {
    sqlx::query(&state.q("DELETE FROM card_labels WHERE card_id = ? AND label_id = ?"))
        .bind(card_id.to_string()).bind(label_id.to_string()).execute(&state.db).await?;
    Ok(())
}

pub async fn list_comments(state: &AppState, _user_id: Uuid, card_id: Uuid) -> Result<Vec<CommentResponse>, AppError> {
    let comments = sqlx::query_as::<_, (String, String, String, String, String, String, String, String)>(
        &state.q(r#"SELECT c.id, c.user_id, u.username, u.display_name,
               COALESCE(u.avatar_url,'') AS avatar_url, c.content,
               COALESCE(c.edited_at,'') AS edited_at, c.created_at
...
pub async fn update_comment(state: &AppState, user_id: Uuid, _card_id: Uuid, comment_id: Uuid, content: String) -> Result<CommentResponse, AppError> {
    sqlx::query(&state.q("UPDATE comments SET content = ?, edited_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?"))
        .bind(&content).bind(comment_id.to_string()).bind(user_id.to_string()).execute(&state.db).await?;
...
pub async fn delete_comment(state: &AppState, user_id: Uuid, _card_id: Uuid, comment_id: Uuid) -> Result<(), AppError> {
    sqlx::query(&state.q("DELETE FROM comments WHERE id = ? AND user_id = ?"))
        .bind(comment_id.to_string()).bind(user_id.to_string()).execute(&state.db).await?;
    Ok(())
}

pub async fn list_checklists(state: &AppState, _user_id: Uuid, card_id: Uuid) -> Result<Vec<ChecklistResponse>, AppError> {
...
pub async fn create_checklist(state: &AppState, _user_id: Uuid, card_id: Uuid, title: String) -> Result<ChecklistResponse, AppError> {
...
pub async fn update_checklist(state: &AppState, _user_id: Uuid, _card_id: Uuid, checklist_id: Uuid, title: String) -> Result<ChecklistResponse, AppError> {
    sqlx::query(&state.q("UPDATE checklists SET title = ? WHERE id = ?"))
        .bind(&title).bind(checklist_id.to_string()).execute(&state.db).await?;
    let items = sqlx::query_as::<_, (String, String, i64, i32, String, String)>(
        &state.q("SELECT id, title, is_checked, position, COALESCE(assigned_to,'') AS assigned_to, COALESCE(due_date,'') AS due_date FROM checklist_items WHERE checklist_id = ? ORDER BY position")
    )
...
pub async fn delete_checklist(state: &AppState, _user_id: Uuid, _card_id: Uuid, checklist_id: Uuid) -> Result<(), AppError> {
    sqlx::query(&state.q("DELETE FROM checklists WHERE id = ?")).bind(checklist_id.to_string()).execute(&state.db).await?;
    Ok(())
}

pub async fn create_checklist_item(state: &AppState, _user_id: Uuid, _card_id: Uuid, checklist_id: Uuid, req: CreateChecklistItemRequest) -> Result<ChecklistItemResponse, AppError> {
    let pos = sqlx::query_scalar::<_, i32>(
    ...
    Ok(ChecklistItemResponse {
        id,
        title: req.title,
        is_checked: false,        position: pos,
        assigned_to: req.assigned_to,
        due_date: req.due_date,
    })
}

pub async fn update_checklist_item(state: &AppState, _user_id: Uuid, _card_id: Uuid, _checklist_id: Uuid, item_id: Uuid, req: UpdateChecklistItemRequest) -> Result<ChecklistItemResponse, AppError> {
    if let Some(title) = &req.title {
        sqlx::query(&state.q("UPDATE checklist_items SET title = ? WHERE id = ?")).bind(title).bind(item_id.to_string()).execute(&state.db).await?;
    }
    if let Some(checked) = req.is_checked {
        sqlx::query(&state.q("UPDATE checklist_items SET is_checked = ? WHERE id = ?")).bind(if checked { 1i64 } else { 0i64 }).bind(item_id.to_string()).execute(&state.db).await?;
    }

    let item = sqlx::query_as::<_, (String, String, i64, i32, String, String)>(
        &state.q("SELECT id, title, is_checked, position, COALESCE(assigned_to,'') AS assigned_to, COALESCE(due_date,'') AS due_date FROM checklist_items WHERE id = ?")
    )
    .bind(item_id.to_string()).fetch_one(&state.db).await?;
...
pub async fn delete_checklist_item(state: &AppState, _user_id: Uuid, _card_id: Uuid, _checklist_id: Uuid, item_id: Uuid) -> Result<(), AppError> {
    sqlx::query(&state.q("DELETE FROM checklist_items WHERE id = ?")).bind(item_id.to_string()).execute(&state.db).await?;
    Ok(())
}

pub async fn list_attachments(state: &AppState, _user_id: Uuid, card_id: Uuid) -> Result<Vec<AttachmentResponse>, AppError> {
    let attachments = sqlx::query_as::<_, (String, String, String, i64, String, String)>(
        &state.q("SELECT id, filename, file_url, file_size, COALESCE(mime_type,'') AS mime_type, created_at FROM attachments WHERE card_id = ?")
    )
    .bind(card_id.to_string()).fetch_all(&state.db).await?;
...
pub async fn delete_attachment(state: &AppState, _user_id: Uuid, _card_id: Uuid, attachment_id: Uuid) -> Result<(), AppError> {
    sqlx::query(&state.q("DELETE FROM attachments WHERE id = ?")).bind(attachment_id.to_string()).execute(&state.db).await?;
    Ok(())
}

pub async fn get_custom_field_values(state: &AppState, _user_id: Uuid, card_id: Uuid) -> Result<Vec<serde_json::Value>, AppError> {
...
pub async fn set_custom_field_value(state: &AppState, _user_id: Uuid, card_id: Uuid, req: SetCustomFieldValueRequest) -> Result<serde_json::Value, AppError> {
...
pub async fn archive_card(state: &AppState, _user_id: Uuid, card_id: Uuid) -> Result<(), AppError> {
...
pub async fn complete_card(state: &AppState, _user_id: Uuid, card_id: Uuid) -> Result<(), AppError> {
    sqlx::query(
