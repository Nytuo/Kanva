...
/**
 * Desktop-only auto-updater UI. No-op on web and mobile — the embedded
 * updater commands only exist in the desktop Tauri binary.
 */
export default function UpdaterModal() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const isDesktop = await invoke<boolean>('is_desktop').catch(() => false);
      if (cancelled || !isDesktop) return;
      setEnabled(true);

      const { listen } = await import('@tauri-apps/api/event');

      const a = await listen<UpdateInfo>('updater-update-available', (event) => {
      ...
      const p = await listen<ProgressPayload>('updater-progress', (event) => {
          ...
          <DialogHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <ArrowDownCircle className="h-5 w-5 shrink-0 text-primary" />
                <DialogTitle>
          ...
          <div className="flex flex-col gap-4">
            {updateInfo?.body && phase === 'idle' && (
              <div className="max-h-48 overflow-y-auto rounded-md bg-muted px-3 py-2 text-sm">
                <pre className="whitespace-pre-wrap break-words font-sans">{updateInfo.body}</pre>
              </div>
            )}
